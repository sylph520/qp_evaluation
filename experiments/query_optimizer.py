...
#     choose next N query plans and add to experience
    ...
    def select_plans(self, model, get_batch):
        ...
        latss = [[self.latencies[i][qid]
                  for i in range(self.arms)] for qid in qids]
        best_lats = 0
        post_lats = 0
        sel_lats = 0
...
look_back = 800
freq = 100
bo_agent = BanditOptimizer(planss, rootss, latencies,
...
for steps in range(len(latencies[0])//freq):
    ...
    train(model, loader, loader, dat[1],
          ds_info, args, prints=False, record=False)
