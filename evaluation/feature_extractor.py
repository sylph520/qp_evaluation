...
class DatasetInfo:
    ...
    def construct_from_plans(self, plans):
        ...
        self.card_norm = Normalizer(
            np.log(float(min_card) + 0.001), np.log(float(max_card) + 0.001))
        self.cost_norm = Normalizer(
            np.log(float(min_cost) + 0.001), np.log(float(max_cost) + 0.001))
        self.cost_est_norm = Normalizer(
            np.log(float(min_cost_est) + 0.001), np.log(float(max_cost_est) + 0.001))
        # for plan cost
        self.width_norm = Normalizer(
            np.log(float(min_width) + 0.001), np.log(float(max_width) + 0.001))
...
def formatConds(conds, table):  # either filter or join
    ...
    for cond in conds:
        ...
        # join
        if len(cond) == 3 and not isinstance(cond[2], list) and not cond[2].isnumeric():
            ...
            if cond[3] == '::timestamp':
                d = datetime.strptime(cond[2].split(
                    "'")[1], '%Y-%m-%d %H:%M:%S')
            else:
